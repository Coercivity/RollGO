...
namespace LobbyAPI.Hubs
{
    public class LobbyHub(LobbyManager lobbyManager) : Hub
    {
        ...
        public async Task JoinLobby(Guid lobbyId)
        {
            ...
            Guid userGuid = Guid.Parse(Context.User?.FindFirst(x => x.ValueType == ClaimTypes.NameIdentifier)!.Value!);
            await _lobbyManager.JoinLobby(lobbyId, userGuid, Context.ConnectionId);

            await Groups.AddToGroupAsync(Context.ConnectionId, lobbyId.ToString());
            await Clients.Group(lobbyId.ToString()).SendAsync(LobbyActions.UserJoined, Context.ConnectionId);
        }
        ...
        public async Task JoinLobbyAnonymous(Guid lobbyId, string userName)
        {
            ...
            await Groups.AddToGroupAsync(Context.ConnectionId, lobbyId.ToString());
            await Clients.Group(lobbyId.ToString()).SendAsync(LobbyActions.AnonymousUserJoined, Context.ConnectionId);
        }
    }
}
