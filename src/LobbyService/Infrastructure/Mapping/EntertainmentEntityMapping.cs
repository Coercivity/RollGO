...
{
    internal class EntertainmentEntityMapping() : BaseEntityMapping<EntertainmentEntity>(nameof(EntertainmentEntity))
    {
        public override void Configure(EntityTypeBuilder<EntertainmentEntity> builder)
        {
            builder.HasDiscriminator<bool>(x => x.IsSeries)
                .HasValue<Film>(false)
            ...
            builder.Property(x => x.RatingImdb)
                .HasColumnName("RatingImdb");

            builder.Property(x => x.RatingKinopoisk)
